...
> forefst's `files`/`details` subcommands are the forensic-grade equivalents (41-column CSV/JSON, owner+group SID, hard-link names+counts, snapshot counts, reparse targets, FileRef/FileId/HomeOid join keys, IsSparse). refsanalysis keeps a lighter, human-readable lab view.
